...
`genesis-q4-core` has zero runtime dependencies on other GenesisAeon
packages — it is foundational (Tier T0). If this ever changes, pin any
ecosystem dependency with `>=` lower bounds matching the minimum version
that provides the API this package relies on. Do not pin exact versions
(`==`) for ecosystem dependencies.
